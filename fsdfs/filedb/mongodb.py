...
class mongodbFileDb(FileDbBase):
    def connect(self):
        self.connection = pymongo.Connection(self.options["host"],
                                             self.options["port"])
    ...
    def update(self, file, data):
        ...
        if "nuked" in data:
            if data["nuked"]:
                ...
                self.nukes.update({"_id": file}, {"$set": {"t": time()}},
        ...
        #print toupdate
        self.files.update({"_id": file}, toupdate, upsert=True,
        ...
        if updatekn:
            self.files.update({"_id": file}, {"$set": {"kn": self.getKn(file)}},
                              safe=True, multi=False)
    ...
    def temporaryIncrementKn(self, file):
        self.files.update({"_id": file}, {"$set": {"kn": self.getKn(file)+1}},
                          safe=True, multi=False)
    ...
    def addFileToNode(self, file, node):
        self.files.update({"_id": file}, {"$addToSet": {"nodes": node}},
                          safe=True)
        self.hasChanged = True
    ...
    def listInNode(self, node):
        return set([
            f["_id"] for f in self.files.find({"nodes": node}, fields=["_id"])
        ])
    ...
    def getMaxKnInNode(self, node, num=1):
        ...
        to write
        '''
        return [f["_id"] for f in self.files.find(
            {"nodes": node},
            sort=[("kn", -1)],
            limit=num,
            fields=["_id"]
    ...
    def getMinKnNotInNode(self, node):
        file = list(self.files.find({"$nor": [{"nodes": {"$size": 0}},
                                    {"nodes":node}]},
                                    limit=1,
                                    fields=["_id"]))
    ...
    def getSizeAll(self):
        ...
        #size = self.files.map_reduce(SIZE_MAP, SIZE_REDUCE, 
        #    out=self.prefix + "_mapreduce_getsizeall",
        #    query={"nuked": {"$exists": False}}
        #).find_one({"_id": "size"})
    ...
    def getSizeInNode(self, node):
        # Cache this because it's an expensive operation that slowed down bulk
        # imports a lot
        if node in self.cacheSizeInNode:
            return self.cacheSizeInNode[node]

        size = self.files.group({}, {"nodes":node}, {"sumsize": 0},
                                GROUP_SIZE_REDUCE)

        #size = self.files.map_reduce(SIZE_MAP, SIZE_REDUCE,
        #    out=self.prefix + "_mapreduce_getsizeinnode",
        #    query={"nuked": {"$exists": False},
        #        "nodes":node}
        #).find_one({"_id": "size"})
