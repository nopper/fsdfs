...
class RPCServer(Thread):
    def __init__(self, fs):
        ...
        self.server = ThreadingBaseHTTPServer(
            (re.split(":", fs.host)[0], int(re.split(":", fs.host)[1])),
            myHandler
        )
        ...
        try:
            ...
            if p[1] == "DOWNLOAD":
                ...
                if not self.server.fs.ismaster:
                    ...
                    # Answer before because addNode can take lots of time if
                    # getting first packet with_files
                    self.simpleResponse(200, "ok")
            ...
            self.simpleResponse(500, str(exc))
            self.server.fs.error("When serving %s %s : %s" % \
                                 (self.path, params, exc))
